...
exports.downloadVideo = catchAsync(async (req, res, next) => {
    ...
    // const url = "http://localhost:8080/api/videoUpload/video"

    // test if url points to a url
    if (!url || !(/https?.*?\.mp4/g.test(url))) {
        next(new AppError("Please provide a valid url that points to a video", 404))
    } else {

        // If all passed then download video and save to files/downloads
        https.get(url, function (res) {
            const fileStream = fs.createWriteStream(`files/downloads/download-${Date.now()}.mp4`)
            res.pipe(fileStream);
